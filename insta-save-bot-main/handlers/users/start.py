        ...
        if malumot[2] == id:
            if  malumot[3] == 'ru':
                await message.answer(f"Отправьте мне ссылку на Instagram – ролики, посты, медиа\n"
                                     "\n"
                                     f"Администратор - @Kasimov_1007\n"
                                     f"Свяжитесь с @Kasimov_1007, чтобы создать бота")

            elif  malumot[3] == 'en':
                await message.answer(f"Send me Instagram link - Reels, Posts , Media\n"
                                     f"\n"
                                     f"Admin - @Kasimov_1007 \n"
                                     f"Contact @Kasimov_1007 to Create a Bot")
            elif malumot[3] == 'uz':
                await message.answer(f"Menga Instagram havolasini yuboring - Reels, Posts , Media\n"
                                     f"\n"
                                     f"Admin - @Kasimov_1007 \n"
                                     f"Bot Yaratish uchun @Kasimov_1007 ga Murojaat Qiling")
